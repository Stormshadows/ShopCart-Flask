...
def search():
    with sqlite3.connect('database.db') as conn:
        ...
        if request.method == "POST":
            ...
            # search by item
            cur.execute("SELECT name, description,price,image from products WHERE name = ? ", (searchQuery, ))
            conn.commit()
...
def root():
    ...
    app.logger.info(categoryData)
    app.logger.info(itemData)
...
def checkoutForm():
    if 'email'  in session:
        ...
        for row in products:
            totalPrice += row[2]
    return render_template("checkoutpage.html", products = products, totalPrice=totalPrice, loggedIn=loggedIn, firstName=firstName, noOfItems=noOfItems)
